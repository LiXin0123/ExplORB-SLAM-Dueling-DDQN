...
class DDQNAgent:
    def __init__(self, model_path, gazebo_env, gamma, learning_rate, epsilon, epsilon_min, epsilon_decay,
        ...
        self.folder_path = f'/home/lx23/Downloads/explORB-SLAM-RL/src/decision_maker/src/python/RL/models/{gazebo_env}/ddqn'
        # Create directory if it does not exist
        if not os.path.exists(self.folder_path):
            os.makedirs(self.folder_path)

        self.folder_path_plot = f'/home/lx23/Downloads/explORB-SLAM-RL/src/decision_maker/src/python/RL/plots/{gazebo_env}/ddqn'
        # Create directory if it does not exist
    ...
    def prepare_input(self, robot_pos, robot_orie, centr, info):
        """Prepares the input for the DDQN model."""
        # Convert the NumPy arrays to PyTorch tensors and move them to the correct device
    ...
    def get_max_info_gain_centroid(self):
        ...
        # 确保 max_info_gain_centroid 被移动到正确的设备上
        return max_info_gain_centroid.to(self.device), max_info_gain_centroid_idx
    ...
    def predict_centroid(self, robot_position, robot_orientation, centroid_records, info_gain_records):
        ...
        # 准备输入并确保所有张量在同一设备上
        network_input, _, sorted_centroid_record = self.prepare_input(
            torch.tensor(robot_position, device=self.device),
            torch.tensor(robot_orientation, device=self.device),
            torch.tensor(centroid_records, device=self.device),
            torch.tensor(info_gain_records, device=self.device)
        )
