...
class DuelingDDQN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply ReLU activation to the value stream's first fully connected layer output
        value = torch.relu(self.value_stream_fc1(x))
        # Obtain the value stream's final output
        value = self.value_stream_fc2(value)

        # Apply ReLU activation to the advantage stream's first fully connected layer output
        advantage = torch.relu(self.advantage_stream_fc1(x))
        # Obtain the advantage stream's final output
...
class DuelingDDQNAgent:
    def __init__(self, model_path, gazebo_env, gamma, learning_rate, epsilon, epsilon_min, epsilon_decay,
        ...
        # Initialize visit_count for centroids (all start at 0)
        self.visit_count = np.zeros(len(self.centr_arr2), dtype=int)
        ...
        self.folder_path = f'/home/lx23/Downloads/explORB-SLAM-RL/src/decision_maker/src/python/RL/models/{gazebo_env}/dueling_ddqn'
        # Create directory if it does not exist
        if not os.path.exists(self.folder_path):
            os.makedirs(self.folder_path)

        self.folder_path_plot = f'/home/lx23/Downloads/explORB-SLAM-RL/src/decision_maker/src/python/RL/plots/{gazebo_env}/dueling_ddqn'
        # Create directory if it does not exist
    ...
    def prepare_input(self, robot_pos, robot_orie, centr, info, visit_count):
        """Prepares the input and computes necessary features for the Dueling DDQN model."""
        ...
        # 计算距离代价（例如使用欧式距离）
        distance_cost = torch.norm(robot_position - sorted_centroid_record, p=2, dim=-1).mean()

        # 计算探索增益
        exploration_gain = 1 / (visit_count + 1)
        ...
        return network_input, output_size, sorted_centroid_record, distance_cost, exploration_gain
    ...
    def initialize_dueling_ddqn(self):
        ...
        network_input, output_size, _, _, _ = self.prepare_input(
            self.robot_post_arr, self.robot_orie_arr, self.centr_arr, self.info_arr, 0
        )
        self.dueling_ddqn = DuelingDDQN(
            network_input.shape[1], output_size).to(self.device)
        self.target_dueling_ddqn = DuelingDDQN(
            network_input.shape[1], output_size).to(self.device)
    ...
    def update_target_network(self):
        ...
        self.target_dueling_ddqn.load_state_dict(
            self.dueling_ddqn.state_dict())
    ...
    def load_model(self):
        ...
        self.dueling_ddqn.load_state_dict(torch.load(
            self.filepath, map_location=self.device))
        self.target_dueling_ddqn.load_state_dict(
            torch.load(self.filepath, map_location=self.device))
    ...
    def select_action(self, state, output_size, sorted_centroid_record):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                q_values = self.dueling_ddqn(state).clone()

                # Get the indices of the centroids which are [0.0, 0.0]
                indices = (sorted_centroid_record == torch.tensor(
                    [0.0, 0.0], device=self.device)).all(1).nonzero(as_tuple=True)[0]
    ...
    def calculate_reward(self, predicted_centroid, best_centroid, info_gain, distance_cost, exploration_gain, visit_penalty):
        """Calculates the reward for a given centroid based on multiple factors."""
        reward = 0

        # Match between predicted and best centroid
        if torch.all(torch.eq(predicted_centroid, best_centroid)):
            reward += 1

        # Apply penalty if predicted centroid is [0.0, 0.0]
        zero_centroid = torch.tensor([0.0, 0.0], device=self.device)
        if torch.all(torch.eq(predicted_centroid, zero_centroid)):
            reward -= self.penalty

        # Apply distance cost penalty
        reward -= 0.5 * distance_cost

        # Increase for exploration gain
        reward += 0.25 * exploration_gain

        # Penalize for visiting frequently
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for i in range(len(self.robot_post_arr2) - 1):
                self.load_model()
                network_input, output_size, sorted_centroid_record, distance_cost, exploration_gain = self.prepare_input(
                    self.robot_post_arr2[i], self.robot_orie_arr2[i], self.centr_arr2[i], self.info_arr2[i], self.visit_count[i]
                )
                ...
                # 定义目标质心
                best_centroid = torch.tensor(self.best_centr_arr2[i], dtype=torch.float32, device=self.device)

                # 计算奖励
                rewards = self.calculate_reward(
                    predicted_centroid, best_centroid, self.info_arr2[i], distance_cost, exploration_gain, self.visit_count[i])

                # Save next state
                next_state, _, _, next_distance_cost, next_exploration_gain = self.prepare_input(
                    self.robot_post_arr2[i + 1], self.robot_orie_arr2[i + 1], self.centr_arr2[i + 1], self.info_arr2[i + 1], self.visit_count[i + 1]
                )
    ...
    def get_max_info_gain_centroid(self):
        ...
        network_input, _, sorted_centroid_record, _, _ = self.prepare_input(
            self.robot_post_arr, self.robot_orie_arr, self.centr_arr, self.info_arr, 1
        )
        with torch.no_grad():
    ...
    def predict_centroid(self, robot_position, robot_orientation, centroid_records, info_gain_records):
        ...
        """Finds the centroid with the highest information gain."""
        network_input, _, sorted_centroid_record, _, _ = self.prepare_input(
            robot_position, robot_orientation, centroid_records, info_gain_records, 1
        )
